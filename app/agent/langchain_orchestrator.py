...
class AgenticComplianceOrchestrator:
    """Fully agentic compliance AI orchestrator."""
    ...
    # ✅ Ensure plan is never empty (fallback)
        if not plan:
            plan.append(PolicyGoalPlan(goal="ANALYZE policy compliance", steps=[StepResult(
                Risk="UNKNOWN",
                Controls=[],
                Recommendation="No steps generated",
                Data_Requirements=[]
            )]))
    ...
    # ----------------- Agent Decision -----------------
    def agent_decide(self, state: dict) -> dict:
        prompt = f"""
You are an autonomous compliance agent.
State:
...
Decide the next action to reach the compliance audit goal.
...
Generate 2-4 compliance goals as a JSON array of strings.
Rules:
- Output JSON only
- No markdown or extra text
Example: ["Ensure labor law compliance", "Maintain HR records"]
"""
        res = query_llm(prompt)
        try:
            goals = json.loads(res)
            if isinstance(goals, list) and goals:
                return goals
        except Exception:
            pass
        # fallback goal if LLM fails
        return ["ANALYZE policy compliance"]

    def plan_steps(self, goal: str, context: str) -> List[StepResult]:
        prompt = f"""
You are a compliance planning AI.

Context: {context}
Goal: {goal}

Generate 2-4 steps in JSON matching this schema:

{StepResult.schema_json(indent=2)}
"""
        res = query_llm(prompt)
        try:
            steps_data = json.loads(res)
            step_results = []
            ...
            # fallback placeholder step
            return [StepResult(
                Risk="UNKNOWN",
                Controls=[ControlItem(
                    Control_Object=f"{goal} Control",
                    Control_Type="Preventative",
                    Purpose=f"Mitigate risks for {goal}",
                    Scope=ControlScope(
                        Department="All",
                        Department_Role="Responsible",
                        Department_Role_Scope="Full",
                        Document_used="Policy Document"
                    )
                )],
